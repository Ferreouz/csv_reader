...
class CSVReader
{
    ...
    {
        $this->header = str_getcsv(mb_convert_encoding($this->rawData[0], 'UTF-8', mb_detect_encoding($this->rawData[0], ['UTF-8', 'ISO-8859-1'])), $this->headerSeparator);
        return $this->header;
    }
    private function constructData(): array
    {
        ...
        foreach ($this->rawData as $index => $line) {
            ...
            $this->data[] = str_getcsv(mb_convert_encoding($line, 'UTF-8', mb_detect_encoding($line, ['UTF-8', 'ISO-8859-1'])), $this->dataSeparator);
        }
        return $this->data;
    }
